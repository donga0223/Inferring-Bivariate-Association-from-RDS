...
permutation.test.II <- function(net, fix.variable, permute.variable, test, n, correct = T)
{
  ...
  for(j in 1:n){
    ...
    #order.net.with.rec[[new.permute.variable]] <- NA
    
  }
    
  #hist(per.test.res, xlim = c(min(per.test.res, obs.test.res$statistic), max(per.test.res, obs.test.res$statistic)))  
  #abline(v = obs.test.res$statistic, col = 2, lwd = 2)
...
permutation.test.both.II <- function(net, fix.variable, permute.variable, test, n, correct = T)
{
  if(is.rds.data.frame(net)==FALSE){
    stop(sprintf("net must be an rds.data.frame"))
  }
